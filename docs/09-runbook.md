...
- **`availability_reason`** (machine-readable, `null` when available) on every `GET /chains` route, and `sol_to_glc_availability_reason` on `/status` and `/stats`: the highest-ranked gate that refused, in the evaluator's own names — `liquidity_buffer_low`, `route_admission_closed`, `reserve_admission_closed`, `reserve_paused`, `utxo_liquidity_low`, `insufficient_capacity` — plus `route_disabled`, `not_implemented`, `onchain_paused`, `reserve_unavailable`, `probe_unavailable`, `destination_limit_unavailable` (a capped route whose `destination_admissible_atomic` could not be derived — docs/40-destination-bound-admission.md), and on `/status` only `quota_exhausted`. `unavailable_reason` keeps its cause-agnostic end-user copy unchanged.
- **`max_transfer_atomic` / `max_transfer_display`** on every `GET /chains` route (and on `GET /limits`, `POST /quote`): the SOURCE transfer limit — 50 000 GLC (`min_transfer::SOURCE_MAXIMUM_CANONICAL`), capped only by the source chain's own deposit ceiling. `POST /quote` / `POST /transfers` refuse a larger amount `400 above_source_maximum`; a chain-sourced deposit above it folds parked (`above source maximum`, operator-recoverable). **`destination_admissible_atomic` / `destination_capacity_sufficient`**: what the destination can settle right now against its per-transfer payout cap at the live rate with `destination_limit_buffer_bps` to spare — a settlement-capacity figure, never the user's limit; `POST /quote` / `POST /transfers` refuse a larger amount `400 destination_payout_out_of_bounds` (both figures in the body) BEFORE any row or deposit address exists (docs/40-destination-bound-admission.md, 2026-09-20).
